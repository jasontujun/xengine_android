...
/**
 * 调用WeakReference内容T的.equals()方法进行比较。
 ...
 */
public class XWeakEqualsReference<T> extends WeakReference<T> {
    public XWeakEqualsReference(T r) {
        super(r);
    }

    public XWeakEqualsReference(T r, ReferenceQueue<? super T> q) {
        super(r, q);
    }
    ...
    public boolean equals(Object other) {
        ...
        if (!returnValue && (other instanceof XWeakEqualsReference<?>)) {
            T value = this.get();
            if (null != value) {
                T otherValue = ((XWeakEqualsReference<T>) other).get();
